    ...
    Politicians.findOne({
        ...
        if (err) {
            ...
            if (doc === null) {
                ...
                console.log('DB SAVE MONGO CALLED');
                doc.firstName = requestQuery.address.firstName;
                doc.lastName = requestQuery.address.lastName;
                doc.Street = requestQuery.address.Street;
                doc.City = requestQuery.address.City;
                doc.State = requestQuery.address.State;
                doc.Zip = requestQuery.address.Zip;
                doc.Phone = requestQuery.address.Phone;
                doc.email = requestQuery.address.email;
                doc.contact = requestQuery.address.contact;
                doc.save();
                response.status(200).send({
                    result: 'success',
